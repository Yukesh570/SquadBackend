...
from squadServices.serializer.connectivitySerializer.connectivitySerializer import ConnectivitySerializer
...
class ConnectivityFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Connectivity
        fields = ['smppHost', 'smppPort', 'systemID', 'smtpUser','bindMode','sourceTON','destTON','sourceNPI','destNPI','createdAt']
...
class ConnectivityViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if exist.exists():
            raise ValidationError({"error": "Connectivity with this systemID already exists."})
    ...
    def perform_update(self, serializer):
        ...
        if systemID != serializer.instance.systemID:
            exist = Connectivity.objects.filter(systemID__iexact=systemID, isDeleted=False)
            if exist.exists():
                raise ValidationError({"error": "Connectivity with the same systemID already exists."})
        user = self.request.user
